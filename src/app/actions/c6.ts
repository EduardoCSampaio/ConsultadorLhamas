...
export async function verificarStatusAutorizacaoC6(input: z.infer<typeof getStatusSchema>): Promise<C6StatusResult> {
    const validation = getStatusSchema.safeParse(input);
    if (!validation.success) {
        return { success: false, message: 'Dados de entrada inválidos.' };
    }
